...
def plot_all(users, results, outdir):
    ...
    for m in METRICS:
        ...
        mask = ~np.isnan(lo)
        plt.fill_between(xs[mask], lo[mask], hi[mask], alpha=0.5, label="CI (train)",color = "green")
        # plt.plot(xs[mask], mean[mask], label="Train Mean")
        plt.plot(xs[mask], val_mean[mask], linestyle='-', label="Validation Mean",color = "red", linewidth = 0.8)
        plt.xlabel("Users")
        plt.ylabel(m)
        plt.title(f"{m}: CI (train) vs Validation Mean")
        plt.grid()
        plt.legend()
    ...
    for m in ['Throughput', 'Goodput', 'Badput']:
        ...
        val_mean = np.array(results[m]["val_mean"])
        mask = ~np.isnan(lo)
        plt.fill_between(xs[mask], lo[mask], hi[mask],
                        color=colors[m], alpha=0.5)
        # # train mean
        # plt.plot(xs[mask], mean[mask],
        #          color=colors[m], linewidth=1.5,
        #          label=f"{m} (train)")
        # validation mean
        plt.plot(xs[mask], val_mean[mask],
                linestyle='-', color=col[(count+1)%3],
                label=f"{m} (val)", linewidth=0.8)
        count+=1
    plt.xlabel("Users")
    plt.ylabel("Requests/sec")
    plt.title("Throughput / Goodput / Badput (CI + Validation Mean)")
    plt.grid()
    plt.legend()
