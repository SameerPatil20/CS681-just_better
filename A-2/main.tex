...
\begin{itemize}
    \item \textbf{Independent Replications:} 20 runs with different RNG seeds

    \item \textbf{Point Estimate:} Mean of the metric across all 20 runs for each user count

    \item \textbf{99\% Confidence Interval:}
    \[
    CI = \bar{X} \pm t_{0.005,19} \cdot \frac{S}{\sqrt{20}}
    \]
    where $S$ is the sample standard deviation across runs, and 
    $t_{0.005,19} \approx 2.861$ (Student’s t-distribution)
    ...
    \item More the number of runs, narrower the confidence interval, but also higher the computational cost. We chose 20 runs as a balance between statistical confidence and practical runtime.
\end{itemize}
...
\frame{
    ...
    \begin{center}
        \includegraphics[width=0.8\textwidth]{confidence_interval/plots/ResponseTime.png}
    \end{center}
    \small
    \begin{itemize}
...
\frame{
    ...
    \begin{center}
        \includegraphics[width=0.8\textwidth]{confidence_interval/plots/Throughput.png}
    \end{center}
    \small
    \begin{itemize}
        \item Throughput increases with users up to saturation point
        \item Peak throughput $\approx$ 72 requests/sec at moderate load
        \item Plateaus after saturation due to server saturation
    \end{itemize}
}

\frame{
    \frametitle{Setup 2: Goodput vs Badput : 1}
    \begin{center}
        \includegraphics[width=0.7\textwidth]{confidence_interval/plots/throughput_goodput_badput.png}
    \end{center}
    \small
    \begin{itemize}
        \item \textbf{Goodput:} The goodput plot follows the throughput plot as initially none of the requests time out.Goodput is non-zero at high load because short or lucky requests, aided by fair scheduling and finite timeouts, still manage to complete before their deadlines—even in a congested system.
        
        \end{itemize}
}
\frame{
    \frametitle{Setup 2: Goodput vs Badput : 2}
    \begin{center}
        \includegraphics[width=0.7\textwidth]{confidence_interval/plots/throughput_goodput_badput.png}
    \end{center}
    \small
...
\frame{
    ...
    \begin{center}
        \includegraphics[width=0.8\textwidth]{confidence_interval/plots/Util.png}
    \end{center}
    \small
    \begin{itemize}
        \item Core utilization increases with user load until saturation
        \item Reaches $\approx 98\%$ at saturation (Did not achieve 100\% utilization because of resources used in context-switches)
    \end{itemize}
}
...
\frame{
    ...
    \begin{itemize}
        ...
        \item \textbf{High load: }Timeouts saturates as more requests are dropped due to full queue, preventing further timeouts
    \end{itemize}
}
...
\frame{
    ...
    \begin{itemize}
        ...
        \item \textbf{Confidence}: 99\% CI indicates statistical soundness
    \end{itemize}
}
...
\frame{
    ...
    \begin{columns}[T]
        ...
        \begin{itemize}
            \item \texttt{simulator.h/cpp}: Main event loop
