...
\frame{
    ...
    \begin{itemize}
        ...
        \item Discrete event simulation with proper warmup and transient removal
...
\frame{
    ...
    \begin{enumerate}
        \item \textbf{Initialization}: Create cores, thread pool, initialize RNG
        \item \textbf{User Arrivals}: Users generate requests in closed loop
...
\frame{
    ...
    \begin{itemize}
        \item \textbf{Average Response Time}: Mean time from request arrival to completion
...
% ========================================
\section{Setup 1: MVA Comparison with Assignment 1 Baseline}
...
\frame{
    ...
    The plot compares:
    \begin{itemize}
        \item \textbf{MVA}: Theoretical prediction line
        \item \textbf{Assignment 1}: Measured data from real system
        \item \textbf{Simulation}: Discrete event simulation results
    \end{itemize}
}
...
\frame{
    \frametitle{Setup 2: Statistical Methodology}
    \begin{itemize}
        \item \textbf{Independent Replications}: 40 runs with different RNG seeds
        \item \textbf{Point Estimate}: Mean of metric across all 40 runs for each user count
        \item \textbf{95\% Confidence Interval}: 
            $$\text{CI} = \bar{X} \pm t_{0.025,39} \cdot \frac{s}{\sqrt{40}}$$
            where $t_{0.025,39} \approx 2.02$ (Student's t-distribution)
        \item \textbf{Warmup Removal}: First 20 seconds discarded (Welch's procedure informal)
        \item \textbf{Transient Analysis}: Statistics only from steady-state period
    \end{itemize}
}
...
\frame{
    ...
    \begin{itemize}
        ...
        \item Sharp increase in response time and confidence interval at high loads
        \item Saturation point near 1500-2000 users
    \end{itemize}
}
...
\frame{
    ...
    \begin{itemize}
        ...
        \item Peak throughput $\approx$ 16-17 requests/sec at moderate load
        \item Plateaus after saturation due to thread pool limits
    \end{itemize}
}
...
\frame{
    ...
    \small
    \begin{itemize}
        \item Goodput: Requests completed before timeout
...
\frame{
    ...
    \small
    \begin{itemize}
        \item Core utilization increases with user load
        \item Reaches $\approx 95\%$ at saturation
        \item 4-core system efficiently utilized
    \end{itemize}
}
...
\frame{
    ...
    \begin{itemize}
        \item \textbf{Saturation Point}: System becomes resource-constrained around 1500-2000 users
        ...
        \item \textbf{Confidence}: 95\% CI indicates statistical soundness
    \end{itemize}
}
...
\frame{
    ...
    \begin{itemize}
        ...
        \item Is SJF better than RR for web server workloads?
        \item What is the trade-off between fairness and latency?
    \end{itemize}
...
\frame{
    ...
    \begin{itemize}
        \item \textbf{Uniform Service Time}: Creates variability for SJF to optimize
        \item \textbf{Multiple Cores}: Real-world scenario with parallelism
        \item \textbf{Larger Quantum}: Reduces context-switching overhead impact
        \item \textbf{Higher CS Overhead for SJF}: Penalty for more frequent preemptions
...
\frame{
    ...
    \small
    \begin{itemize}
        \item SJF generally shows lower response time at moderate loads
        \item Both policies degrade similarly at high load (resource saturation dominates)
        \item Trade-off: SJF fairness vs. average latency
    \end{itemize}
}
...
\frame{
    ...
    \small
    \begin{itemize}
        \item Throughput curves nearly identical for RR and SJF
        \item Scheduling policy has minimal impact on aggregate throughput
        \item Bottleneck is thread pool and core resources, not scheduling
    \end{itemize}
}
...
\frame{
    ...
    \begin{itemize}
        ...
        \item \textbf{Trade-off}: Higher context-switching overhead for SJF
...
\frame{
    ...
    \begin{itemize}
        \item \textbf{Simulator Validation}: Discrete event simulation closely matches MVA theory
        \item \textbf{Performance Characterization}: Comprehensive metrics with confidence intervals
        \item \textbf{Scheduling Impact}: Limited impact on aggregate throughput at saturation
        \item \textbf{Saturation Point}: System saturates around 1500-2000 users for given config
        \item \textbf{Statistical Rigor}: Proper use of confidence intervals and warmup removal
        \item \textbf{Extensibility}: Framework supports multiple scheduling policies, distributions, and workloads
    \end{itemize}
}
...
\frame{
    ...
    \begin{columns}[T]
        ...
        \begin{itemize}
            ...
            \item \texttt{mix\_plot.py}: Multi-source comparison
